...
async fn setup(
    ...
    run_git(&["config", "http.uploadpack", "true"], &bare_repo);
    t.setup("Created test repo with receivepack+uploadpack enabled");
    ...
    // Clear the request log so per-test assertions only see traffic from
    // the test's own actions, not the clone in setup().
...
/// Returns true if the upstream received a POST to `/repo.git/git-upload-pack`
/// after the given point. Used to assert whether the sidecar negotiation fired.
fn upload_pack_posts(log: &RequestLog) -> usize {
    log.lock()
        .unwrap()
        .iter()
        .filter(|e| e.starts_with("POST ") && e.contains("/git-upload-pack"))
        .count()
}
...
async fn test_force_push_to_protected_branch_blocked() {
    ...
    t.assert_true(
        "sidecar negotiation fired (upload-pack POST to upstream)",
        upload_pack_posts(&request_log) > 0,
    );
...
async fn test_fast_forward_to_protected_branch_allowed() {
    ...
    // Pack walk confirms FF locally for a pushed new commit — sidecar must NOT fire.
    t.assert_eq(
        "pack-walk-only path: no sidecar upload-pack POST",
        &upload_pack_posts(&request_log),
        &0usize,
    );
...
async fn test_force_push_to_unprotected_branch_allowed() {
    ...
    // Ref not in protected list — no ancestry check runs at all.
    t.assert_eq(
        "non-protected ref: no sidecar upload-pack POST",
        &upload_pack_posts(&request_log),
        &0usize,
    );
...
async fn test_delete_protected_branch_blocked() {
    ...
    // Deletion is handled purely from pkt-line inspection — no ancestry check.
    t.assert_eq(
        "deletion path: no sidecar upload-pack POST",
        &upload_pack_posts(&request_log),
        &0usize,
    );
...
async fn test_create_matching_protected_branch_allowed() {
    ...
    // Create is allowed without any ancestry check.
    t.assert_eq(
        "create path: no sidecar upload-pack POST",
        &upload_pack_posts(&request_log),
        &0usize,
    );
...
async fn test_protected_pattern_wildcard() {
    ...
    let posts_after_force = upload_pack_posts(&request_log);
    t.assert_true(
        "sidecar fired on force-push to release/v1",
        posts_after_force > 0,
    );
...
/// Pack walk alone can't confirm a fast-forward when `new-sha` is a commit that
/// already exists on the server (via another ref) — the client's thin pack is
/// effectively empty, so there are no parent edges to walk. The sidecar
/// want/have negotiation against the upstream is what keeps this case from
/// being a false-positive block.
///
/// Repro: server starts with `main` → C1. Client clones, creates a `feature`
/// branch with commit C2 on top of C1, and pushes `feature` (so server now has
/// both `main=C1` and `feature=C2`). Then, still on the client, we ask git to
/// update `main` on the server to point at C2 — a genuine fast-forward whose
/// new commit is already server-known. Pack is empty; pack walk says
/// `NotAncestor`; sidecar ACKs; push is allowed.
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn test_sidecar_allows_ff_to_commit_already_on_server() {
    let t = test_report!(
        "FF to a commit that already exists server-side (via another ref) — sidecar ACKs"
    );
    let Setup {
        proxy,
        upstream,
    ...
    // Create feature/x on top of main with one commit, push it to origin.
    run_git(&["checkout", "-b", "feature/x"], &clone_dir);
    std::fs::write(clone_dir.join("shared.txt"), "shared commit\n").unwrap();
    run_git(&["add", "shared.txt"], &clone_dir);
    run_git(&["commit", "-m", "shared commit"], &clone_dir);
    ...
    t.assert_eq(
        "feature/x initial push succeeds",
        &output.status.code().unwrap(),
        &0,
    );

    // Snapshot the upload-pack POST count. The sidecar fires only on the NEXT
    // push (the fast-forward-via-existing-commit one), not the clone/fetch path.
    let baseline_posts = upload_pack_posts(&request_log);

    // Fast-forward main to point at C2 (the commit already on server via feature/x).
    // The pack sent to the server will contain no new objects — C2 is already there.
    let output = run_command_reported(
        &t,
        git_with_proxy(
    ...
    t.assert_eq(
        "ff to existing commit succeeds (sidecar ACK)",
        &output.status.code().unwrap(),
        &0,
    );

    let posts_after = upload_pack_posts(&request_log);
    t.assert_true(
        "sidecar negotiation fired for this ambiguous FF",
        posts_after > baseline_posts,
    );

    // Verify bare repo now has main pointing at C2.
    let verify = std::process::Command::new("git")
        .args(["rev-parse", "main"])
        .current_dir(&bare)
        .output()
