...
impl CredentialTemplate {
    ...
    /// Like [`CredentialTemplate::resolve`], but with an injectable variable
    /// lookup. See [`resolve_credential_value_with`] for why tests need this.
    pub fn resolve_with(&self, lookup: impl Fn(&str) -> Option<String>) -> Result<String> {
        match self {
            ...
            CredentialTemplate::Variable {
                ...
                let v = lookup(var_name).ok_or_else(|| {
                    Error::config(format!(
